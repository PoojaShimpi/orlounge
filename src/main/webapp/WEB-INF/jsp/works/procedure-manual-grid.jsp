...
<section class="panel">
    ...
    <h2 class="panel-title clearfix">Administrative Manual lists

        <%
            if(token.isLSA() || token.isAdmin() || token.isORM()){
        %>
        <a class="btn btn-primary pull-right"  href="viewprmanual.html?action=<%=AppConstants.ACTION_CREATE%>" >Create OR Administration Manual<%--<i class="fa fa-plus"></i>--%></a>
        <%
            }
        %>
    ...
    <div class="panel-body">
            ...
            <table class="table table-bordered table-striped mb-none" id="datatable-default" >
                ...
                <tbody>
                <%
                    List<ProcedureManualBean> scheduleBeans= (List<ProcedureManualBean>) request.getAttribute("manuals");
                    for(ProcedureManualBean u : scheduleBeans){
                        boolean allowedToUpdate = token.isAdmin() || token.isLSA() || token.isORM();
                        %>
                <tr class="gradeX" >
                    <td>
                        <a title="View" href="viewprmanual.html?id=<%=u.getId()%>&action=<%=AppConstants.ACTION_VIEW%>">
                            <%=u.getTopic()%>
                        </a>
                    </td>
                    <td>
                        ...
                        <%
                        if(allowedToUpdate){ %>
                         <a title="View" href="viewprmanual.html?id=<%=u.getId()%>&action=<%=AppConstants.ACTION_UPDATE%>">
                            Edit
                        </a> |
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="remove_manual.html?id=<%=u.getId()%>" class="btn btn-danger btn-ok">Delete</a>
                                    </div>
                                </div>
                            </div>
                        </div>
