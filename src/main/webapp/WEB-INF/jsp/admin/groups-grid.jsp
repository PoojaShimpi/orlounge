...
--%>
    ...
    <h2 class="panel-title">ORLS List</h2>
</header>
...
<tr class="gradeX">
    ...
    <td><%=u.getNoOfBeds()%></td>
    <td><%=u.getUser().getFirstName()+" " + u.getUser().getLastName()%> </td>
    <td><%=u.getUser().getHospitalTelPh()%></td>
    <td><%=u.getUser().getHospitalTelExt()%></td>
    <%
        if(u.getLocations().isEmpty()){
            %>
            <td  class="success">No Restriction</td>
    <%
        }else {
    %>
    <td  class="danger">Geo Restricted</td>
    <%
        }
    %>
    ...
    <td class="actions-hover actions-fade">
        ...
        <a href="deleteUser?userId=<%=u.getUserId()%>" class="delete-row"><i class="fa fa-trash-o"></i></a>
        <a href="staff/view?id=<%=u.getUserId()%>" class="delete-row"><i class="fa fa-eye"></i></a>--%>
    </td>
    <%
        } %>
    <% if(u.getIsActive().equals(-1)){
